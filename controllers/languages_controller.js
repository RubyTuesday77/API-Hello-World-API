...
// Seed with language data (go to http://localhost:3000/languages/seed):
...
// Select a random language (go to http://localhost:3000/languages/random):
languages.get('/random', async (req, res) => {
    let count = await Language.countDocuments()  // Count how many documents are available
    let random = Math.floor(Math.random() * count)  // Select a random document number
    Language.findOne().skip(random)  // Find one document and skip to the randomly generated number above
        .then(foundLanguage => {
